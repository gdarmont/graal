...
public final class ProfilerStackWalkVisitor extends ParameterizedStackFrameVisitor<Void> {
    @Override
    ...
    protected boolean visitFrame(Pointer sp, CodePointer ip, CodeInfo codeInfo, DeoptimizedFrame deoptimizedFrame, Void voidData) {
        return ProfilerSampleWriter.putLong(ProfilerThreadLocal.getWriterData(), ip.rawValue());
    }

    @Override
    @Uninterruptible(reason = "Called from uninterruptible code.", mayBeInlined = true)
    protected boolean unknownFrame(Pointer sp, CodePointer ip, DeoptimizedFrame deoptimizedFrame, Void data) {
        /* We should not fail fatally as we are still inside the signal handler. */
        return false;
    }
}
